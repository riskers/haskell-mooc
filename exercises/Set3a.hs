...
firstHalf s
  | even l = take (div l 2) s
  | otherwise = take (div (l+1) 2) s
  where l = length s
...
capitalize s = unwords (map firstUpper (words s))

firstUpper::String -> String
firstUpper (x:s) = toUpper x : s
...
while check update value
  | check value =  while check update newValue
  | otherwise = value
  where newValue = update value
...
joinToLength n xs = filter validLen [ x++y | x <- xs, y <- xs]
  where validLen s = length s == n
...
multiApp f gs x = f (map (\ff -> ff x) gs)
...
interpreter xs = interpreter' xs 0 0 []
  where
    interpreter' [] _ _ c = c
    interpreter' ("up":xs) x y c = interpreter' xs x (y+1) c
    interpreter' ("down":xs) x y c = interpreter' xs x (y-1) c
    interpreter' ("left":xs) x y c = interpreter' xs (x-1) y c
    interpreter' ("right":xs) x y c = interpreter' xs (x+1) y c
    interpreter' ("printY":xs) x y c = interpreter' xs x y (c ++ [show y])
    interpreter' ("printX":xs) x y c = interpreter' xs x y (c ++ [show x])

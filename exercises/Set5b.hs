...
isOrdered (Node x l r) = isLeftOrdered x l && isRightOrdered x r && isOrdered l && isOrdered r
  where
    isLeftOrdered _ Empty = True
    isLeftOrdered v (Node lv _ _) = v > lv
    isRightOrdered _ Empty = True
    isRightOrdered v (Node rv _ _) = v < rv
...
walk (StepL: xs) (Node v l _) = walk xs l
walk (StepR: xs) (Node v _ r) = walk xs r
...
set (StepL:xs) _ (Node v Empty r) = Node v Empty r
set (StepR:xs) _ (Node v l Empty) = Node v l Empty
...
search x (Node v l r)
  | x == v = Just []
  | otherwise = case search x l of
      Just a -> Just (StepL: a)
      Nothing -> case search x r of
        Just a -> Just (StepR: a)
        Nothing -> Nothing

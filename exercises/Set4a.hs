...
longest :: (Foldable t, Ord (t a)) => [t a] -> t a
longest [x] = x
longest (x:y:xs)
...
incrementKey _ [] = []
incrementKey key maps = map (\(k, v) -> if k == key then (k, v+1) else (k,v)) maps
...
winner scores player1 player2 = if p1score < p2score then player2 else player1
  where
    p1score = Map.lookup player1 scores
    p2score = Map.lookup player2 scores
...
freqs = foldr update Map.empty
  where
    update:: (Ord a) => a -> Map.Map a Int -> Map.Map a Int
    update key = Map.insertWith (+) key 1
...
maxIndex arr = fst (minimumBy largest (assocs arr))
  where
    largest x y = compare (snd y) (snd x)
